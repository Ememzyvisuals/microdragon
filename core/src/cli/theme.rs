...
impl Theme {
    pub fn bold_str(s: &str) -> String {
        use crossterm::style::Stylize;
        s.to_string().bold().to_string()
    }
    pub fn muted_str(s: &str) -> String {
        use crossterm::style::Stylize;
        s.to_string().with(Self::DIM).to_string()
    }
}
...
impl Theme {
    pub fn banner() -> String {
        use crossterm::style::Stylize;
        "🐉 MICRODRAGON".to_string().with(Self::GREEN).bold().to_string()
    }
    pub fn separator() -> String {
        "─".repeat(60).to_string()
    }
    pub fn glyph_arrow() -> &'static str { "→" }
    pub fn glyph_microdragon() -> &'static str { "🐉" }
