...
impl SelfImprovementLedger {
    ...
    pub fn record_failure(&mut self, error: &str, context: &str) {
        ...
        // Collect data before any mutable borrow ends — avoids E0499
        let existing = self.failure_patterns.iter()
            .find(|(p, _)| p == &pattern)
            .map(|(_, count)| *count);
        ...
        match existing {
            Some(count) => {
                ...
                // Now borrow is released — safe to call suggest_improvement
                if new_count >= 3 {
                    let obs = format!("Recurring failure: '{}' ({}x)", pattern, new_count);
                    let sug = format!("Add handling for '{}' in context: {}", pattern,
                                      &context[..context.len().min(100)]);
                    self.suggest_improvement(&obs, &sug);
                }
            }
            None => {
